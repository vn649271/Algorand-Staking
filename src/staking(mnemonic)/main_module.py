...
def start_staking(stakeholderMnemonic, amountInMicroAlgo):
    ...
    stakeholderAlgosBefore = getBalances(algodClient, stakeholder.getAddress())[0]
    print("Algo balance for ", stakeholder.getAddress(), ": ", stakeholderAlgosBefore, " algos")
    print("Staking is being about to start: ", amountInMicroAlgo / 1000000, " algos")

    return placeStake(
        client = algodClient, 
        appID = appID, 
        stakeholder = stakeholder, 
        stakeAmount = amountInMicroAlgo
    )
...
def cancel_stake(stakeholderMnemonic, amount):
    stakeholder = getStakeholderAccount(algodClient, stakeholderMnemonic)
    return unstaking(algodClient, appID, stakeholder, amount)
