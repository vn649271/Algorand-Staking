...
from pyteal import *
from dummy_contract import *

# user declared account mnemonics
creator_mnemonic = "vague glow either salmon immense mask model crystal tone flee veteran what tackle gallery entire wrap segment tortoise earth video inquiry unique orange abstract whip"
...
# call application
def call_app(client, stakeholder_address, stakeholder_mnemonic, staking_amount, index, app_args) : 
    ...
    # create unsigned transaction
    #saveStakingInfoTxn = transaction.ApplicationNoOpTxn(stakeholder, params, index, app_args)
    ...
    # sign transaction
    signedSaveStakingTxn = saveStakingInfoTxn.sign(stakeholder_private_key)
    signedPaymentTxn = paymentTxn.sign(stakeholder_private_key)

    #tx_id = signedSaveStakingTxn.transaction.get_txid()

    # send transaction
    client.send_transactions([signedSaveStakingTxn])
    # await confirmation
    wait_for_confirmation(client, saveStakingInfoTxn.get_txid(), 5)
    
    # send transaction
    client.send_transactions([signedPaymentTxn])
    # await confirmation
    wait_for_confirmation(client, paymentTxn.get_txid(), 5)
...
def main_func() :
    ...
    algod_client = algod.AlgodClient(
        algod_token, 
        algod_address, 
        # headers={'User-Agent': 'bWGapCpvM04fLKChEQ0fW1GZk3dCyznj9Ks3r7ud'}
    )
