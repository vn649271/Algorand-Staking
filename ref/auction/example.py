...
def simple_auction():
    ...
    creator = getCreatorAccount(client)
    seller = getSellerAccount(client)

    print("Alice is generating an example NFT...")
    nftAmount = 1
    nftID = createDummyAsset(client, nftAmount, seller)
    print("The NFT ID is:", nftID)
    startTime = int(time()) + 60  # start time is 10 seconds in the future
    endTime = startTime + 180  # end time is 30 seconds after start
    ...
    print(
        "Alice is creating auction smart contract that lasts 180 seconds to auction off NFT..."
    )
    ...
    bidder = getBidderAccount(client)
